...
/**
 * Exclui fisicamente múltiplos objetos do Cloudflare R2 por suas chaves (keys) ou URLs públicas
 */
export async function deleteR2Objects(objectKeysOrUrls = []) {
  ...
  console.log(`[R2 Physical Deletion] Excluindo ${keys.length} fotos do bucket '${bucketName}':`, keys);
  ...
  try {
    ...
    console.log(`✅ [R2 Physical Deletion Success] ${keys.length} fotos excluídas fisicamente do Cloudflare R2.`);
  } catch (err) {
    console.warn('⚠️ [R2 Batch Delete Warning] Tentando exclusão individual...', err.message);
    
    for (const key of keys) {
      try {
        const singleCommand = new DeleteObjectCommand({
          Bucket: bucketName,
          Key: key,
        });
        await r2Client.send(singleCommand);
        console.log(`✅ [R2 Single Delete Success] Foto excluída fisicamente: ${key}`);
      } catch (singleErr) {
        console.error(`❌ [R2 Single Delete Error] Falha ao excluir ${key}:`, singleErr.message);
      }
    }
  }
}
